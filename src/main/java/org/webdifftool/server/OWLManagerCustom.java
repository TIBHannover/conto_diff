...
//		mergedMap.putAll(formatRight.asPrefixOWLDocumentFormat().getPrefixName2PrefixMap());
	...
	public static StringBuilder getAutoGeneratedHeader(OWLOntology ontology) {
		...
		if (ontology.getOntologyID().getOntologyIRI().isPresent() && ontology.getOntologyID().getVersionIRI().isPresent()) {
			strBuilder.append("\t<").append(ontology.getOntologyID().getOntologyIRI().get()).append("> rdf:type owl:Ontology ;\n");
			strBuilder.append("\t\towl:versionIRI <").append(ontology.getOntologyID().getVersionIRI().get()).append("> ;\n");
			strBuilder.append("\t\trdfs:comment \"").append(ontology.getOntologyID().getOntologyIRI().get()).append("\" .\n\n\n");
		}
		return strBuilder;
	}
