...
public class OWLManagerCustom {
					...
					for (OWLClass cExpr: expr.getEntities()) {
						if (!cExpr.isAnonymous()){
							...
							if (relValue!=null&&relName!=null) {
								objRelSet.addRelationship(relValue, importObj.getAccessionNumber(), "rdfs:subClassOf");
							} else {
	...
	public static Map<String, String> getAllPrefixes(OWLOntology ontology) {
	...
	public static String[] replaceURIsWithPrefixes(String input, Map<String, String> prefixMap) {
		StringBuilder output = new StringBuilder(input);
		for (Map.Entry<String, String> entry : prefixMap.entrySet()) {
			String prefix = entry.getKey();
			String uri = entry.getValue();
			int index = output.indexOf(uri);
			while (index != -1) {
				output.replace(index, index + uri.length(), prefix);
				index = output.indexOf(uri, index + prefix.length());
			}
		}
		String[] parts = output.toString().split(" ");
		String hashOperation = parts[0] + "_" + parts[1];
		String[] result = new String[parts.length - 1];
		result[0] = hashOperation;
		System.arraycopy(parts, 2, result, 1, parts.length - 2);
		if (parts.length == 3) {
			output.append(" rdf:type owl:Class .");
		}
		return result;
	}

	public static void getProvDmMap(Map<String, String> prefixes, String header, String input, Map<String, String> provDmMap) {
		String[] output = replaceURIsWithPrefixes(input, prefixes);
		StringBuilder stringBuilder = new StringBuilder();
		stringBuilder.append(header).append(System.lineSeparator());
		stringBuilder.append(Arrays.toString(output)).append(System.lineSeparator());

		provDmMap.put(output[0], stringBuilder.toString());
	...
	public static void generateActivityMap(Map<String, String> locations, Map<String, String> activities) {
		for (Map.Entry<String, String> entry : locations.entrySet()) {
			String key = entry.getKey().substring(0, entry.getKey().lastIndexOf("_"));
			String value = ":" + key + " rdf:type owl:NamedIndividual ,\n\t " +
					"prov:Activity ;\n\t" +
					"prov:addConcept <http://doi.org/10.1610/history-iao-extracted#9d784dbb4810548940b76aff4a8790d6ad379bf6> ;\n\t" +
	...
	public static void generateSDiffEntity(Map<String, String> sourceEntities, Map<String, String> sdiffEntities, String baseEntity) {
		for (Map.Entry<String, String> entry : sourceEntities.entrySet()) {
			...
			String value = key + "> rdf:type owl:NamedIndividual ,\n\t " +
					...
					"prov:generatedAtTime \"2023-11-30T10:58:44.000Z\"^^xsd:dateTime ;\n\t" +
					"prov:value \"update iao-extracted with journal article imported without hierarchy\" ;\n\t" +
					"rdfs:label \"" + sha1 + "\" ;\n\t" +
